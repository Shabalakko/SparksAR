...
public class PowerUpQuestionUI : MonoBehaviour
{
    ...
    private IEnumerator QuestionTimerCoroutine()
    {
        ...
        if (!answered)
        {
            ...
            expiredMessageText.text = "";
            HandleTimeout(); // Changed to direct call, was a co-routine.
        }
    }


    private void HandleTimeout() // Changed to void, was IEnumerator
    {
        if (answered) return;
        answered = true;
    ...
    private void CheckAnswer(int index)
    {
        ...
        if (index == correctAnswerIndex)
        {
            ...
            currentPowerUp.GrantPowerUp(true); // Passa true per indicare risposta corretta
        }
        else
        {
            answerButtons[index].image.color = wrongColor;
            currentPowerUp.GrantPowerUp(false); // Passa false per indicare risposta errata
        }

        StartCoroutine(ResetUIAfterDelay()); // Added this
    }
